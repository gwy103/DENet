...
class Segmentationd(Dataset):
    NUM_CLASSES = 1

    def __init__(self, args, split='train'):
        super(Segmentationd, self).__init__()
        ...
        self._edge_dir = os.path.join(self._base_dir, 'train','crops','edges')
        self.args = args
        ...
        for splt in self.split:
            with open(os.path.join(os.path.join(_splits_dir,'train', splt + '_crops.txt')), "r") as f:
                lines = f.read().splitlines()

            for ii, line in enumerate(lines):
                _image = os.path.join(self._image_dir, line + '.png')
                _cat = os.path.join(self._cat_dir, line + '.png')
                _ed = os.path.join(self._edge_dir, line + '.png')
