...
class Segmentation_testd(Dataset):

    def __init__(self, args, split='test'):

        super(Segmentation_testd, self).__init__()
        self._base_dir = Path.db_root_dir(args.dataset)
        self._image_dir = os.path.join(self._base_dir, 'test','crops', 'images')
        self._cat_dir = os.path.join(self._base_dir,'test', 'crops','gt')
        self._edge_dir = os.path.join(self._base_dir, 'test','crops', 'edges')
        self.args = args
        ...
        for splt in self.split:
            with open(os.path.join(os.path.join(_splits_dir, 'test',splt + '_crops.txt')), "r") as f:
    ...
    def _make_img_gt_point_pair(self, index):
        ...
        _target = Image.open(self.categories[index]).convert('L')  # 转为灰色图像
